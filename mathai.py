...
def byparts(f, g, depth, var, data):
    eq = integratex(g, depth-1, var, data+[TreeNode("f_int", [solve(expand2(g)), tree_form(var)])])
    if eq is None:
        return None
    
    tmp2 = diffx(f, var) * eq
    eq2 = integratex(tmp2, depth-1, var, data+[TreeNode("f_int", [solve(expand2(tmp2)), tree_form(var)])])
...
def handle_intrec(special, equation):
    ...
    if "f_int" in str_form(equation):
        ...
        #return None
    return equation
def integratex(equation, depth, var="v_0", data=[]):
    ...
    if special2 in [str_form(item) for item in data[:-1]]:
        return special
    ...
    if const != tree_form("d_1"):
        ...
        r = integratex(tmp4, depth-1, var, data+[special])
    ...
    for eq in lst:
        ...
        if eq.name == "f_add":
            ...
            s = integratex(eq.children[0], depth - 1, var, data+[special])
            ...
            if s is not None:
                for child in eq.children[1:]:
                    tab += 1
                    plog(f"{'  '*tab}integating over sums")
                    u = integratex(child, depth - 1, var, data+[special])
    ...
    if tmp3 is not None:
        ...
        if tmp[5] == tmp[3]:
            tmp6 = integratex(tmp[2]**tmp[4]*tmp[0]*(tmp[1]+tmp[2]**(tree_form("d_-1")*tmp[3]))**(tmp[4]/tmp[3]),depth - 1,  var, data+[special])
            return tmp6
    ...
    for item in collec(eq, var):
        ...
        tmp3 = subs1(equation, x, var, v2, depth, data+[special])
    ...
    if eq.name == "f_mul" and len(eq.children) == 2:
        ...
        tmp4 = byparts(f, g, depth, var, data+[special])
        tab -= 1
...
def inverse2(eq, var="v_0"):
    ...
    return tmp
def subs1(equation, term, v1, v2, depth, data):
    ...
    for eq in lst:
        ...
        tmp = integratex(eq, depth - 1, origv2, data)
        if tmp is None:
            continue
...
def quadratic(equation, integration=False, to_compute=None, root=False):
    output = None
    def quad(eq):
        nonlocal to_compute
        nonlocal root
        ...
        if tmp is not None:
            ...
            if eq.name == "f_add":
                ...
                if tmp is not None:
                    ...
                    d = solve(expand2(d))
                    if compute(d)>=0 or to_compute is not None:
...
while True:
    ...
    if True:
        ...
        if tmp == "factor":   
            ...
            var = {"x":"v_0", "y":"v_1", "z":"v_2", "c": "v_5"}[tmp[-1]]
            special = TreeNode("f_int", [equation, tree_form(var)])
            tmp2 = integratex(equation, 5, var)
